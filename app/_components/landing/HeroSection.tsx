...
const HeroSection = () => {
  ...
  // LCP-friendly animations: Start from nearly-visible state (opacity: 0.01) with NO delay
  // This makes content render immediately for LCP while still having smooth animations
  const containerVariants = {
    hidden: { opacity: 0.01 }, // Nearly visible = good LCP
    visible: {
      opacity: 1,
      transition: {
        duration: 0.6,
        delay: 0, // NO DELAY for LCP
  ...
  const itemVariants = {
    hidden: { opacity: 0.01, y: 10 }, // Minimal movement, nearly visible
    visible: {
      opacity: 1,
      y: 0,
  ...
  const floatingIcons = [
    { icon: BookOpen, delay: 2, x: 100, y: 50 },
    { icon: Brain, delay: 2.5, x: -80, y: 80 },
    { icon: Zap, delay: 3, x: 120, y: -60 },
    { icon: Sparkles, delay: 3.5, x: -100, y: -40 }
  ]
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-orange-50 via-white to-amber-50 dark:from-slate-950 dark:via-slate-900 dark:to-orange-950/20">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {/* Floating Icons - Heavily delayed to not affect LCP */}
      ...
      <div className="container mx-auto px-3 sm:px-4 lg:px-6 py-16 sm:py-20 relative z-10">
        {/* LCP-Optimized Animation: Starts from nearly-visible state with NO delay */}
        <motion.div 
          ...
          animate="visible"
        >
          {/* Badge - Animated but starts nearly visible */}
          <motion.div variants={itemVariants} className="mb-6 sm:mb-8">
            <div className="inline-flex items-center gap-2 bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium font-outfit">
              <img 
                ...
                width="16" 
                height="16" 
                className="w-3 h-3 sm:w-4 sm:h-4 dark:hidden" 
                loading="eager"
                decoding="sync"
              />
              <img 
                ...
                width="16" 
                height="16" 
                className="w-3 h-3 sm:w-4 sm:h-4 hidden dark:block" 
                loading="eager"
                decoding="sync"
              />
          ...
          {/* Main Headline - Animated but starts nearly visible */}
          <motion.h1 
          ...
          {/* Subtitle - LCP Element - Animated but starts nearly visible for good LCP */}
          <motion.p 
            variants={itemVariants}
          ...
          {/* Credit Highlight - Animated but starts nearly visible */}
          <motion.div variants={itemVariants} className="mb-6 sm:mb-8 px-4 sm:px-0">
          ...
          {/* CTA Button - Animated but starts nearly visible */}
          <motion.div
            variants={buttonVariants}
            className="flex justify-center items-center mb-8 sm:mb-12 px-4 sm:px-0"
          >
            {isSignedIn ? (
              <Link href="/dashboard" className="w-full sm:w-auto" prefetch={true}>
                <Button
                  size="lg"
              ...
              <Link href="/sign-up" className="w-full sm:w-auto" prefetch={true}>
                <Button
                  size="lg"
                  className="w-full sm:w-auto bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white border-0 px-8 sm:px-16 py-4 sm:py-6 text-lg sm:text-xl font-semibold rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 font-outfit min-w-0 sm:min-w-[280px]"
                >
      ...
      {/* Scroll Indicator - Heavily delayed */}
      <motion.div
